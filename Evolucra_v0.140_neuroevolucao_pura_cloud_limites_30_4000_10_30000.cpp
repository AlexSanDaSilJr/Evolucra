/*
===============================================================================
 VIDA ARTIFICIAL EVOLUTIVA — EVOLUCAO NEURAL v0.140
...
O documento complementar CONTEXTO_IA_VIDA_ARTIFICIAL_v0.139.md descreve as regras
...
[R-REPRO-06] Qualidade >=90% e boa; 100% e excelente. Qualidade afeta apenas a reproducao biologica.
[R-REPRO-07] Nascimento e morte influenciam a evolucao somente por selecao natural; nao existe reward externo.
...
[R-NEURAL-02] Pesos sinapticos sao exclusivamente geneticos. Nao existe plasticidade por reward, eligibility trace ou aprendizado lamarckiano.
[R-NEURAL-03] Em acasalamento, memoria persistente/episodica consolidada pode ser herdada; pesos de conexao mudam apenas por recombinacao e mutacao genetica.
...
[R-NEURAL-09] Cada ligacao possui peso genetico e funcao de transferencia evolutiva (linear, inversa, absoluta, quadratica, seno, limiar, gaussiana ou saturacao).
[R-NEURAL-10] Complexidade neural tem custo metabolico: neuronios, conexoes, recorrencia e memoria persistente/temporal nao sao gratuitos.
...
[R-SAVE-04] PROGRAM_VERSION atual = 139. FORMAT_VERSION continua 3.
...
-----------------------
  ...
  [SEC-NEUROEVOLUTION]  selecao, mutacao e recombinacao neural
...
constexpr Color CARCASS_ROTTEN_COLOR{18,28,72,255};
// [R-FOOD-QUALITY] Planta fornece energia de menor qualidade; carne fresca fornece
// energia mais concentrada e duradoura, mas apodrece rapidamente e fica venenosa.
constexpr float PLANT_FOOD_ENERGY_MULT = 0.78f;
constexpr float PLANT_FOOD_QUALITY = 0.32f;
constexpr float MEAT_FRESH_SECONDS = 5.0f;
constexpr float MEAT_DECAY_END_SECONDS = 15.0f;
constexpr float MEAT_POISON_END_SECONDS = 20.0f;
constexpr float MEAT_FRESH_ENERGY_MULT = 1.45f;
constexpr float MEAT_15S_ENERGY_MULT = 0.85f;
constexpr float MEAT_ROTTEN_ENERGY_MULT = 0.20f;
constexpr float MEAT_15S_QUALITY = 0.55f;
constexpr float MEAT_ROTTEN_QUALITY = 0.08f;
constexpr float MEAT_MAX_POISON_DAMAGE_PER_BITE = 14.0f;
...
struct RuntimeTuning {
    ...
    // A ligacao possui peso genetico e uma funcao evolutiva.
    ...
    // Registradores persistentes controlados pela propria rede.
...
// NeuralConnectionGene::enabled: somente bit0 e usado (ativa/inativa). Bits altos de saves antigos sao descartados.
...
void normalizeNeuralConnectionMeta(NeuralConnectionGene& c){c.enabled=uint8_t(c.enabled&0x01u);}
void setNeuralConnectionEnabled(NeuralConnectionGene& c,bool on){c.enabled=uint8_t(on?1u:0u);}
void copyNeuralConnectionMeta(NeuralConnectionGene& dst,const NeuralConnectionGene& src){
    dst.enabled=uint8_t(src.enabled&0x01u);
...
struct CompiledConnection {
    int sourceIndex = 0;
    float weight = 0.0f;
    int geneIndex = -1; // legado de UI; o motor nao consulta peso aprendido
    BrainLinkFunction function = BrainLinkFunction::Linear;
    float paramA = 1.0f;
    float paramB = 0.0f;
};
...
struct CompiledBrain {
    std::vector<std::vector<CompiledConnection>> hiddenIncoming;
    std::vector<std::vector<CompiledConnection>> outputIncoming;
...
void ensureConnectivity(NeuralGenome& g) {
    ...
    // v0.140: conexoes sao apenas soma ponderada. Remove modo/plasticidade de saves antigos.
    for(auto& c:g.connections) normalizeNeuralConnectionMeta(c);
...
CompiledBrain compileBrain(const NeuralGenome& g) {
    CompiledBrain c;
    c.hiddenIncoming.resize(g.hiddenCount);
    c.outputIncoming.resize(g.outputCount);
    int maxLayer = 0;
    ...
    for (const auto& gene:g.connections) {
        ...
        CompiledConnection cc{sourceIndexFor(gene,recurrent),gene.weight,-1,gene.function,gene.paramA,gene.paramB};
        if(gene.dstKind==BrainNodeKind::Hidden)c.hiddenIncoming[gene.dst].push_back(cc);
        else c.outputIncoming[gene.dst].push_back(cc);
        ++c.activeConnections;
        if(recurrent)++c.recurrentConnections;
    }
    return c;
}
...
NeuralGenome mutateBrain(const NeuralGenome& parent) {
    ...
    for (auto& c : g.connections) {
        ...
        normalizeNeuralConnectionMeta(c);
...
struct Carcass : GridTracked {
    ...
    Carcass(Vector2 p, float size) : pos(p), bodySize(size) {
        ...
        // A carne agora dura exatamente 20 s se nao for comida. Quantidade e qualidade
        // sao conceitos separados: ela nao evapora energeticamente so por envelhecer.
        duration = cfg::MEAT_POISON_END_SECONDS;
        ...
        sensorRadius = std::max(0.08f, visualSize * 0.92f);
        glowRadius = std::max(1.0f, bodySize * cfg::CARCASS_GLOW_SCALE);
        decayPerSecond = 0.0f;
    }

    float spoilT() const { return clampf(age / cfg::MEAT_POISON_END_SECONDS,0.0f,1.0f); }
    float foodQuality() const {
        if(age<=cfg::MEAT_FRESH_SECONDS) return 1.0f;
        if(age<cfg::MEAT_DECAY_END_SECONDS){
            const float t=(age-cfg::MEAT_FRESH_SECONDS)/(cfg::MEAT_DECAY_END_SECONDS-cfg::MEAT_FRESH_SECONDS);
            return 1.0f+(cfg::MEAT_15S_QUALITY-1.0f)*clampf(t,0.0f,1.0f);
        }
        const float t=(age-cfg::MEAT_DECAY_END_SECONDS)/(cfg::MEAT_POISON_END_SECONDS-cfg::MEAT_DECAY_END_SECONDS);
        return cfg::MEAT_15S_QUALITY+(cfg::MEAT_ROTTEN_QUALITY-cfg::MEAT_15S_QUALITY)*clampf(t,0.0f,1.0f);
    }
    float energyMultiplier() const {
        if(age<=cfg::MEAT_FRESH_SECONDS) return cfg::MEAT_FRESH_ENERGY_MULT;
        if(age<cfg::MEAT_DECAY_END_SECONDS){
            const float t=(age-cfg::MEAT_FRESH_SECONDS)/(cfg::MEAT_DECAY_END_SECONDS-cfg::MEAT_FRESH_SECONDS);
            return cfg::MEAT_FRESH_ENERGY_MULT+(cfg::MEAT_15S_ENERGY_MULT-cfg::MEAT_FRESH_ENERGY_MULT)*clampf(t,0.0f,1.0f);
        }
        const float t=(age-cfg::MEAT_DECAY_END_SECONDS)/(cfg::MEAT_POISON_END_SECONDS-cfg::MEAT_DECAY_END_SECONDS);
        return cfg::MEAT_15S_ENERGY_MULT+(cfg::MEAT_ROTTEN_ENERGY_MULT-cfg::MEAT_15S_ENERGY_MULT)*clampf(t,0.0f,1.0f);
    }
    float poisonSeverity() const {
        if(age<=cfg::MEAT_DECAY_END_SECONDS) return 0.0f;
        return clampf((age-cfg::MEAT_DECAY_END_SECONDS)/(cfg::MEAT_POISON_END_SECONDS-cfg::MEAT_DECAY_END_SECONDS),0.0f,1.0f);
    }
    Color currentColor() const {
        const float t=spoilT();
        auto ch=[&](unsigned char a,unsigned char b){return (unsigned char)std::lround(float(a)+(float(b)-float(a))*t);};
        return Color{ch(cfg::CARCASS_COLOR.r,cfg::CARCASS_ROTTEN_COLOR.r),ch(cfg::CARCASS_COLOR.g,cfg::CARCASS_ROTTEN_COLOR.g),ch(cfg::CARCASS_COLOR.b,cfg::CARCASS_ROTTEN_COLOR.b),255};
    }
    ...
    void update(float dt) {
        ...
        sensorRadius=visionRadius();
        resolveCircleWorldBarrier(pos, velocity, radius());
    }
    float radius() const { return std::max(0.08f, visualSize * fraction()); }
    // O raycast entra levemente no desenho para nunca parar numa "bola invisivel" ao redor do PNG.
    float visionRadius() const { return std::max(0.08f, radius()*0.92f); }
    ...
    float bite(float power) { float req = maxBite * clampf(power, 0.0f, cfg::tuning.carcassBitePowerMax); float actual = std::min(req, energy); energy -= actual; sensorRadius=visionRadius(); return actual; }
    bool finished() const { return energy <= 0.01f || age >= cfg::MEAT_POISON_END_SECONDS; }
};
...
struct Creature : GridTracked {
    ...
    float size = 3.0f;
    float energy = 10.0f;
    // Qualidade media da reserva energetica atual. Nao e DNA: e fenotipo nutricional
    // formado pela mistura do que o individuo comeu. 0 = energia curta, 1 = duradoura.
    float energyQuality = 0.60f;
    ...
    // v0.140: nao existe estado sinaptico aprendido/reward em Creature.
    ...
    float absorbFoodEnergy(float offered,float quality){
        const float before=energy;
        const float actual=std::max(0.0f,std::min(offered,maxEnergy-energy));
        if(actual<=0.0f)return 0.0f;
        const float oldMass=std::max(0.0f,before);
        const float q=clampf(quality,0.0f,1.0f);
        energy=before+actual;
        energyQuality=(oldMass+actual)>1e-6f?clampf((oldMass*energyQuality+actual*q)/(oldMass+actual),0.0f,1.0f):q;
        return actual;
    }
    float energyConsumptionQualityMultiplier() const {
        // Planta/energia ruim se esgota mais rapido; reserva de alta qualidade reduz
        // o consumo aparente sem eliminar os custos metabolicos/geneticos do individuo.
        return 1.58f + (0.68f-1.58f)*clampf(energyQuality,0.0f,1.0f);
    }
...
struct PopulationLimits {
    ...
    int minCreatures = 30;
    int maxCreatures = 4000;
    int minPlants = 10;
    int maxPlants = 30000;
};

// [SEC-STATS] Estatisticas, recordes e genealogia
// Estrutura legada mantida SOMENTE porque fazia parte do formato binario ate v0.139.
struct LegacyTrainingRecordBook {
...
static_assert(std::is_trivially_copyable_v<LegacyTrainingRecordBook>);
...
constexpr std::uint32_t PROGRAM_VERSION = 140;
...
struct CreatureState {
    ...
    // Layout legado v0.109+: dados antigos consumidos apenas para retrocompatibilidade.
    std::array<float,cfg::REGISTER_MEMORY_SLOTS> memoryRegisters{};
    std::vector<float> legacyLearnedWeights;
    std::vector<float> legacyEligibilityTraces;
    float legacyLastPlasticSignal=0.0f;
    std::uint64_t legacyTrainingUpdates=0;
    // Layout legado v0.116+: bytes do antigo treinamento; v0.140 apenas preserva alinhamento do save.
    float legacyTrainingPending=0.0f,legacyTrainingTotal=0.0f,legacyTrainingPositive=0.0f,legacyTrainingNegative=0.0f;
    std::uint64_t legacyTrainingEvents=0;
    ...
    float energyQuality=0.60f; // v0.139+: qualidade media da reserva alimentar
};
...
struct EggState {
    ...
    std::vector<float> legacyInheritedWeights;
    std::array<float,cfg::REGISTER_MEMORY_SLOTS> inheritedMemoryRegisters{};
    std::array<LongMemoryEvent,cfg::LONG_MEMORY_SLOTS> inheritedLongMemory{};
...
struct Snapshot {
    ...
    // Payload legado do formato v0.115-v0.139. Apenas alinhamento/compatibilidade; motor v0.140 ignora.
    LegacyTrainingRecordBook legacyTrainingRecords{};
};
...
inline void writeCreature(Writer& w,const CreatureState& c){
    ...
    w.pod(c.memoryRegisters);w.podVector(c.legacyLearnedWeights);w.podVector(c.legacyEligibilityTraces);w.pod(c.legacyLastPlasticSignal);w.pod(c.legacyTrainingUpdates);
    w.pod(c.legacyTrainingPending);w.pod(c.legacyTrainingTotal);w.pod(c.legacyTrainingPositive);w.pod(c.legacyTrainingNegative);w.pod(c.legacyTrainingEvents);
    ...
    w.pod(c.sexualHeritageDepth);w.pod(c.energyQuality);
}
inline void readCreature(Reader& r,CreatureState& c,std::uint32_t programVersion){
    ...
    if(r.ok && programVersion>=108){
        r.pod(c.memoryRegisters);r.podVector(c.legacyLearnedWeights,500000);r.podVector(c.legacyEligibilityTraces,500000);r.pod(c.legacyLastPlasticSignal);r.pod(c.legacyTrainingUpdates);
    }else{
        c.memoryRegisters={};c.legacyLearnedWeights.clear();c.legacyEligibilityTraces.clear();c.legacyLastPlasticSignal=0.0f;c.legacyTrainingUpdates=0;
    }
    if(r.ok && programVersion>=111){
        r.pod(c.legacyTrainingPending);r.pod(c.legacyTrainingTotal);r.pod(c.legacyTrainingPositive);r.pod(c.legacyTrainingNegative);r.pod(c.legacyTrainingEvents);
    }else{
        c.legacyTrainingPending=0.0f;c.legacyTrainingTotal=0.0f;c.legacyTrainingPositive=0.0f;c.legacyTrainingNegative=0.0f;c.legacyTrainingEvents=0;
    }
    ...
    if(r.ok && programVersion>=139)r.pod(c.energyQuality);else c.energyQuality=0.60f;
    c.energyQuality=clampf(c.energyQuality,0.0f,1.0f);
}
...
inline bool writeSnapshot(const std::string& path,const Snapshot& s,std::string& error){
    ...
    w.pod(s.lineage.valid); if(s.lineage.valid){writeGenome(w,s.lineage.genome);w.pod(s.lineage.generation);w.pod(s.lineage.parentId);w.pod(s.lineage.origin);w.pod(s.lineage.parentSize);w.podVector(s.lineage.legacyLearnedWeights);w.pod(s.lineage.memoryRegisters);w.pod(s.lineage.sexualHeritageDepth);}
    ...
    count(s.eggs.size());for(const auto& e:s.eggs){
        ...
        w.podVector(e.legacyInheritedWeights);w.pod(e.inheritedMemoryRegisters);w.pod(e.hasInheritedMemory);
...
inline bool readSnapshot(const std::string& path,Snapshot& s,std::string& error){
    ...
    r.pod(s.lineage.valid);if(s.lineage.valid){readGenome(r,s.lineage.genome,pv);r.pod(s.lineage.generation);r.pod(s.lineage.parentId);r.pod(s.lineage.origin);r.pod(s.lineage.parentSize);if(pv>=110){r.podVector(s.lineage.legacyLearnedWeights,500000);r.pod(s.lineage.memoryRegisters);}else{s.lineage.legacyLearnedWeights.clear();s.lineage.memoryRegisters={};}if(pv>=133)r.pod(s.lineage.sexualHeritageDepth);else s.lineage.sexualHeritageDepth=0;}
    readWatch(r,s.watch);
    ...
    readCount(n);if(r.ok){s.eggs.resize(n);for(auto& e:s.eggs){
        ...
        if(pv>=110){r.podVector(e.legacyInheritedWeights,500000);r.pod(e.inheritedMemoryRegisters);r.pod(e.hasInheritedMemory);}else{e.legacyInheritedWeights.clear();e.inheritedMemoryRegisters={};e.hasInheritedMemory=false;}
        if(pv>=124){
    ...
    if(r.ok && pv>=115) r.pod(s.legacyTrainingRecords);
    else s.legacyTrainingRecords={};
    ...
    if(pv<118){
        s.legacyTrainingRecords.fastestHealth50ToZero=0.0f;s.legacyTrainingRecords.healthCrashId=-1;
        s.legacyTrainingRecords.fastestEnergy50ToZero=0.0f;s.legacyTrainingRecords.energyCrashId=-1;
    }
...
public:
    ...
    saveio::Snapshot makeSaveSnapshot(const saveio::SessionMeta& meta) const {
        ...
        s.limits=limits; s.evo=evo; s.legacyTrainingRecords={}; s.selectedId=selectedId; s.nextCreatureId=nextCreatureId;
        ...
        if(s.lineage.valid){s.lineage.genome=minimumLineageSeed.genome;s.lineage.generation=minimumLineageSeed.generation;s.lineage.parentId=minimumLineageSeed.parentId;s.lineage.origin=minimumLineageSeed.origin;s.lineage.parentSize=minimumLineageSeed.parentSize;s.lineage.memoryRegisters=minimumLineageSeed.memoryRegisters;s.lineage.sexualHeritageDepth=minimumLineageSeed.sexualHeritageDepth;}
        ...
        for(const Creature* c:creatures){
            ...
            x.vegetationFrictionMult=c->vegetationFrictionMult;x.rootedPlantCover=c->rootedPlantCover;x.pushVelocity=c->pushVelocity;x.angularVelocity=c->angularVelocity;x.contactPressure=c->contactPressure;x.recentImpact=c->recentImpact;x.recentDamage=c->recentDamage;x.recentEnergyGain=c->recentEnergyGain;            x.memoryRegisters=c->memoryRegisters;
            x.matingPartnerId=c->matingPartnerId;x.matingContactTime=c->matingContactTime;x.sexualHeritageDepth=c->sexualHeritageDepth;x.energyQuality=c->energyQuality;
        ...
        for(const Egg* e:eggs){
            saveio::EggState x; x.pos=e->pos;x.genome=e->genome;x.generation=e->generation;x.parentId=e->parentId;x.secondParentId=e->secondParentId;x.minimumReplacement=e->minimumReplacement;x.childSize=e->childSize;x.childMaxEnergy=e->childMaxEnergy;x.radius=e->radius;x.age=e->age;x.incubationDuration=e->incubationDuration;x.initialEnergy=e->initialEnergy;x.energy=e->energy;x.initialBlood=e->initialBlood;x.blood=e->blood;x.maxBite=e->maxBite;x.velocity=e->velocity;x.legacyInheritedWeights.clear();x.inheritedMemoryRegisters=e->inheritedMemoryRegisters;x.inheritedLongMemory=e->inheritedLongMemory;x.inheritedLongMemoryHead=e->inheritedLongMemoryHead;x.inheritedLongMemoryCount=e->inheritedLongMemoryCount;x.hasInheritedMemory=e->hasInheritedMemory;x.qualityScore=e->qualityScore;x.plannedBirthCount=e->plannedBirthCount;x.conceivedWithMate=e->conceivedWithMate;x.sexualHeritageDepth=e->sexualHeritageDepth;s.eggs.push_back(std::move(x));
        }
    ...
    bool applySaveSnapshot(const saveio::Snapshot& s,std::string& error) {
        ...
        minimumLineageSeed={};
        if(s.lineage.valid){minimumLineageSeed.valid=true;minimumLineageSeed.genome=s.lineage.genome;minimumLineageSeed.generation=s.lineage.generation;minimumLineageSeed.parentId=s.lineage.parentId;minimumLineageSeed.origin=s.lineage.origin;minimumLineageSeed.parentSize=s.lineage.parentSize;minimumLineageSeed.memoryRegisters=s.lineage.memoryRegisters;minimumLineageSeed.sexualHeritageDepth=s.lineage.sexualHeritageDepth;}
        ...
        for(const auto& x:s.creatures){
            ...
            c->vegetationFrictionMult=x.vegetationFrictionMult;c->rootedPlantCover=x.rootedPlantCover;c->pushVelocity=x.pushVelocity;c->angularVelocity=x.angularVelocity;c->contactPressure=x.contactPressure;c->recentImpact=x.recentImpact;c->recentDamage=x.recentDamage;c->recentEnergyGain=x.recentEnergyGain;c->longMemory=x.longMemory;c->longMemoryHead=x.longMemoryHead;c->longMemoryCount=x.longMemoryCount;c->grabbedByCreatureId=x.grabbedByCreatureId;c->grabLoadRatio=x.grabLoadRatio;c->speciesId=x.speciesId;            c->memoryRegisters=x.memoryRegisters;
            c->matingPartnerId=x.matingPartnerId;c->matingContactTime=x.matingContactTime;c->sexualHeritageDepth=x.sexualHeritageDepth;c->energyQuality=clampf(x.energyQuality,0.0f,1.0f);
        ...
        for(const auto& x:s.carcasses){
            Carcass* k=carcassPool.create(x.pos,x.bodySize);k->age=clampf(x.age,0.0f,cfg::MEAT_POISON_END_SECONDS);k->duration=cfg::MEAT_POISON_END_SECONDS;k->initialEnergy=x.initialEnergy;k->energy=x.energy;k->maxBite=x.maxBite;k->visualSize=x.visualSize;k->sensorRadius=k->visionRadius();k->glowRadius=x.glowRadius;k->decayPerSecond=0.0f;k->velocity=x.velocity;k->grabbedByCreatureId=x.grabbedByCreatureId;carcasses.push_back(k);
        }
        for(const auto& x:s.eggs){
            Egg* e=eggPool.create(x.pos,x.genome,x.generation,x.parentId,x.initialEnergy,x.initialBlood);e->secondParentId=x.secondParentId;e->minimumReplacement=x.minimumReplacement;e->childSize=x.childSize;e->childMaxEnergy=x.childMaxEnergy;e->radius=0.50f*std::max(cfg::tuning.eggRadiusMin,cfg::tuning.eggRadiusBase+e->childSize*cfg::tuning.eggRadiusSizeScale);e->age=x.age;e->incubationDuration=x.incubationDuration;e->initialEnergy=x.initialEnergy;e->energy=x.energy;e->initialBlood=x.initialBlood;e->blood=x.blood;e->maxBite=std::max(cfg::tuning.eggBiteMin,e->radius*cfg::tuning.eggBiteRadiusScale);e->velocity=x.velocity;e->inheritedMemoryRegisters=x.inheritedMemoryRegisters;e->inheritedLongMemory=x.inheritedLongMemory;e->inheritedLongMemoryHead=x.inheritedLongMemoryHead%cfg::LONG_MEMORY_SLOTS;e->inheritedLongMemoryCount=(std::uint8_t)std::min<int>(x.inheritedLongMemoryCount,cfg::LONG_MEMORY_SLOTS);e->hasInheritedMemory=x.hasInheritedMemory;e->qualityScore=x.qualityScore;e->plannedBirthCount=std::clamp(x.plannedBirthCount,1,10);e->conceivedWithMate=x.conceivedWithMate;e->sexualHeritageDepth=x.sexualHeritageDepth;eggs.push_back(e);
        }
    ...
    Egg* spawnMinimumReplacementEgg(const MinimumLineageSeed& seed) {
        ...
        prepareInheritedMemory(*egg,seed);
    ...
    bool dispatchGpuVision(){
        ...
        for(Carcass* k:carcasses)if(k)countObj(k->pos,k->visionRadius());
        ...
        for(Carcass* k:carcasses)if(k){GpuVisionObject o;o.px=k->pos.x;o.py=k->pos.y;o.radius=k->visionRadius();o.type=3;o.color=packVisionColor(k->currentColor());fillObj(o);}
        ...
        for (size_t r = 0; r < c.rays.size(); ++r) {
            ...
            for (const auto& entry : entries) {
                ...
                carcassGrid.forCellKey(entry.key, [&](Carcass* k){
                    ...
                    const float hit=rayCircle(c.pos,dir,imageInEntry(k->pos),k->visionRadius(),best);
                    if(hit>=0.0f&&hit<best){best=hit;type=RayType::Carcass;targetColor=k->currentColor();plantDetached=0.0f;plantSway=0.0f;targetWorldPos=imageInEntry(k->pos);hasTargetWorldPos=true;}
                });
    ...
    void evaluateBrain(Creature& c) {
        // v0.140: neuroevolucao pura. Cada sinapse usa somente o peso genetico do DNA.
        for (int i = 0; i < cfg::INPUT_COUNT; ++i) c.brainState[cfg::BRAIN_INPUT_BASE + i] = brainInputActive(c.genome.brain,i)?c.inputs[i]:0.0f;
        for (int h = 0; h < c.genome.brain.hiddenCount; ++h) c.brainState[cfg::BRAIN_PREV_HIDDEN_BASE + h] = c.hidden[h];
        for (int o = 0; o < cfg::OUTPUT_COUNT; ++o) c.brainState[cfg::BRAIN_PREV_OUTPUT_BASE + o] = c.netOut[o];
        auto linkSignal=[&](const CompiledConnection& cn){return applyBrainLinkFunction(cn.function,c.brainState[cn.sourceIndex],cn.paramA,cn.paramB)*cn.weight;};
        for (int layer = 0; layer < cfg::MAX_BRAIN_LAYERS; ++layer) {
            for (int h : c.compiled.hiddenByLayer[layer]) {
                ...
                const float candidate = applyHiddenActivation(hiddenNodeActivation(node),sum * node.gain);
                ...
                c.hidden[h] = state; c.brainState[cfg::BRAIN_HIDDEN_BASE + h] = state;
            }
        }
        ...
        float drive = c.netOut[0] - c.netOut[1], turn = c.netOut[3] - c.netOut[2], strafe = c.netOut[5] - c.netOut[4];
        c.brainOut[0]=std::max(0.0f,drive); c.brainOut[1]=std::max(0.0f,-drive);
        c.brainOut[2]=std::max(0.0f,-turn); c.brainOut[3]=std::max(0.0f,turn);
        c.brainOut[4]=std::max(0.0f,-strafe); c.brainOut[5]=std::max(0.0f,strafe);
        ...
        for(int slot=0;slot<cfg::REGISTER_MEMORY_SLOTS;++slot){
            ...
            if(slot<regCap&&brainOutputActive(c.genome.brain,cfg::MEMORY_GATE_OUTPUT_BASE+slot)&&c.netOut[cfg::MEMORY_GATE_OUTPUT_BASE+slot]>=cfg::tuning.brainRegisterWriteGateThreshold)c.memoryRegisters[slot]=clampf(c.netOut[cfg::MEMORY_WRITE_OUTPUT_BASE+slot],0.0f,1.0f);
        }
    }
    ...
    void updateCreature(Creature& c, float dt) {
        ...
        const float brainComplexityCost=(
            ...
            float(brainRegisterCapacity(c.genome.brain))*cfg::tuning.brainRegisterEnergyCostPerSecond)*dt;
        ...
        req *= c.energyConsumptionQualityMultiplier();
    ...
    bool creatureBirthPositionFree(Vector2 p, float childSize) const {
        ...
        if (carcassGrid.anyCircle(p.x, p.y, carcassBroad, [&](Carcass* carcass){
            ...
            const float occupied = std::max(carcass->visionRadius(), carcass->visualSize);
    ...
    void prepareInheritedMemory(Egg& egg,const Creature& parent,const Creature* mate) const {
        // v0.140: pesos das conexoes sao apenas DNA. A heranca nao copia aprendizagem sinaptica.
        ...
        for(int slot=0;slot<childCap;++slot){
            ...
            if(n>0){egg.inheritedMemoryRegisters[slot]=clampf(sum/float(n),0.0f,1.0f);inheritedAny|=std::fabs(egg.inheritedMemoryRegisters[slot])>1e-6f;}
        }
        ...
        if(mate){
            ...
            std::array<Candidate,cfg::LONG_MEMORY_SLOTS*2> candidates{};int count=0;
            auto collect=[&](const Creature& src){
                for(int recent=0;recent<(int)src.longMemoryCount&&count<(int)candidates.size();++recent){
                    ...
                    const float age=std::max(0.0f,simTime-e.time),recency=1.0f/(1.0f+age/60.0f);
            ...
            for(int i=0;i<keep;++i){egg.inheritedLongMemory[(std::size_t)i]=chosen[(std::size_t)i];egg.inheritedLongMemoryHead=(std::uint8_t)((i+1)%cfg::LONG_MEMORY_SLOTS);}
            egg.inheritedLongMemoryCount=(std::uint8_t)keep;if(keep>0)inheritedAny=true;
        }
        egg.hasInheritedMemory=inheritedAny; // campo legado: agora significa apenas memoria herdada.
    }

    void prepareInheritedMemory(Egg& egg,const MinimumLineageSeed& seed) const {
        bool inheritedAny=false;
        egg.inheritedMemoryRegisters.fill(0.0f);egg.inheritedLongMemory={};egg.inheritedLongMemoryHead=0;egg.inheritedLongMemoryCount=0;
        const int childCap=brainRegisterCapacity(egg.genome.brain),sourceCap=brainRegisterCapacity(seed.genome.brain);
        for(int slot=0;slot<std::min(childCap,sourceCap);++slot){egg.inheritedMemoryRegisters[slot]=clampf(seed.memoryRegisters[slot],0.0f,1.0f);inheritedAny|=std::fabs(egg.inheritedMemoryRegisters[slot])>1e-6f;}
        egg.hasInheritedMemory=inheritedAny;
    }
    ...
    bool forceFreeEgg(Creature& parent) {
        ...
        prepareInheritedMemory(*egg,parent,mate);
        eggs.push_back(egg);
        eggGrid.insert(egg);
        ++evo.eggsLaid;
    ...
    bool trySoloReproduction(Creature& parent) {
        ...
        prepareInheritedMemory(*egg,parent,nullptr);
...
// [SEC-REPRODUCTION] Acasalamento por sessao cabeca-com-cabeca.
    ...
    void processBites() {
        ...
        for (Creature* up : creatures) {
            ...
            carcassGrid.forCircle(bp.x, bp.y, br + cfg::tuning.maxCreatureSize * cfg::tuning.carcassSensorScale, [&](Carcass* k){
                ...
                float req = br + k->radius(); float d = toroidalDist2(bp, k->pos);
            ...
            if (bestPlant) {
                ...
                const float gain = potentialEnergy * efficiency * cfg::PLANT_FOOD_ENERGY_MULT;
                ...
                const float actualEnergyGain = c.absorbFoodEnergy(gain,cfg::PLANT_FOOD_QUALITY);
                if(removed>0.0f){
                    ++perf.plantFoodBites;
                ...
                const Color meatColor=bestCarcass->currentColor();
                const float similarity = colorSimilarity(c.genome.mouth.color,meatColor);
                const float efficiency = cfg::tuning.carcassMinColorEfficiency +
                    (1.0f-cfg::tuning.carcassMinColorEfficiency)*similarity;
                const float offered=rawGain*efficiency*bestCarcass->energyMultiplier();
                const float actualGain=c.absorbFoodEnergy(offered,bestCarcass->foodQuality());
                const float poison=bestCarcass->poisonSeverity();
                const float biteFraction=clampf(rawGain/std::max(0.01f,bestCarcass->maxBite),0.0f,1.0f);
                const float poisonDamage=cfg::MEAT_MAX_POISON_DAMAGE_PER_BITE*poison*biteFraction;
                const float hpBefore=c.health;
                c.health=clampf(c.health-poisonDamage,0.0f,cfg::tuning.healthMax);
                if(hpBefore>0.0f && c.health<=0.0f && c.deathCause==DeathCause::Unknown)c.deathCause=DeathCause::Poison;
                c.recentEnergyGain=clampf(c.recentEnergyGain+actualGain*cfg::tuning.energyGainFeedbackScale,0.0f,1.0f);
                if(actualGain>0.001f){
                    rememberEventAt(c,MemoryEventType::Food,actualGain/std::max(0.01f,c.maxEnergy),bestCarcass->pos);
                }
                log(c.id, "Comeu carne: idade " + f2(bestCarcass->age) + "s | qualidade " + f2(bestCarcass->foodQuality()*100.0f) +
                    "% | afinidade " + f2(similarity*100.0f) + "% | energia +" + f2(c.energy-before) +
                    " | reserva Q " + f2(c.energyQuality*100.0f) + "% | veneno " + f2(poison*100.0f) + "%.");
                if(poisonDamage>0.01f)log(c.id,"Carne estragada causou "+f2(poisonDamage)+" de dano por veneno.");
            } else if (bestEgg) {
        ...
        for (int i = (int)eggs.size() - 1; i >= 0; --i) {
            ...
            for(int born=0;born<birthCount;++born){
                ...
                log(parentId,
                    ...
                    ", vida "+f2(child->health)+(egg->hasInheritedMemory?(partnered?", memorias dos dois pais herdadas.":", memoria herdada."):"."));
    ...
    void update(float dt) {
        ...
        a=Clock::now();for(Creature* c:creatures){ updateCreature(*c,dt); }b=Clock::now();perf.movement.sample(ms(a,b));
    ...
    static std::size_t compiledBrainHeapBytes(const CompiledBrain& c){
        ...
        add(c.hiddenIncoming);
        add(c.outputIncoming);
    ...
    std::size_t estimatedNeuralBytes() const{
        ...
        for(const Creature* c:creatures){
            if(!c)continue;
            b+=genomeHeapBytes(c->genome)+compiledBrainHeapBytes(c->compiled)+c->hidden.capacity()*sizeof(float);
        }
...
struct TopControlRects {
    RectF plants{},pause{},kill{},config{},system{},checkpoint{},save{},load{},restart{},follow{},fullscreen{},hud{};
};
...
TopControlRects topControlRects(int sw) {
    ...
    place(r.system,76); place(r.config,70); place(r.checkpoint,70); place(r.save,64); place(r.load,64); place(r.restart,78); place(r.follow,82);
    return r;
}
...
float topControlsBottom(const TopControlRects& r,bool showHud){
    ...
    if(!showHud) return b;
    const RectF* all[]={&r.plants,&r.pause,&r.kill,&r.system,&r.config,&r.checkpoint,&r.save,&r.load,&r.restart,&r.follow};
...
bool topControlHit(const TopControlRects& r, Vector2 p, bool showHud) {
    ...
    return pointIn(r.plants,p)||pointIn(r.pause,p)||
           pointIn(r.kill,p)||pointIn(r.system,p)||pointIn(r.config,p)||pointIn(r.checkpoint,p)||pointIn(r.save,p)||pointIn(r.load,p)||pointIn(r.restart,p)||pointIn(r.follow,p);
}
...
void drawFollowPopup(Simulation& sim,UIState& ui,const TopControlRects& controls,int sw,int sh){
    ...
    DrawText("Modos automaticos de acompanhamento",(int)p.x+14,(int)p.y+34,10,Color{165,180,205,255});
    ...
    y+=42;
    DrawText("Neuroevolucao pura: sem ranking por recompensa.",(int)bx,(int)y,11,Color{180,190,205,255});
}
...
void drawCarcass(const Carcass& k, float zoom) {
    ...
    const Color meatColor=k.currentColor();
    if(IsTextureReady(gSprites.meat) && lod>=1){
        DrawTextureCentered(gSprites.meat,k.pos,r*2.15f,r*2.15f,0.0f,meatColor);
        return;
    }
    Color outline=meatColor;outline.r=(unsigned char)(outline.r*0.48f);outline.g=(unsigned char)(outline.g*0.48f);outline.b=(unsigned char)(outline.b*0.48f);
    DrawCircleV(k.pos, r, outline);
    DrawCircleV(k.pos, std::max(0.30f, r - std::max(0.22f, r * 0.14f)), meatColor);
}
...
std::array<float,cfg::OUTPUT_COUNT> brainSnapshotMotor(const Creature& c,int muteEmotion=-1){
    ...
    auto sig=[&](const CompiledConnection& cn){return applyBrainLinkFunction(cn.function,state[cn.sourceIndex],cn.paramA,cn.paramB)*cn.weight;};
    for(int i=0;i<cfg::INPUT_COUNT;++i){
        float v=c.inputs[i];
...
void drawBrainGraph(const Creature& c, RectF area) {
    ...
    // Ligacoes primeiro. Recorrentes ficam alaranjadas; todas usam peso genetico.
    for(int ci=0;ci<(int)g.connections.size();++ci){
        ...
        const float w=cn.weight;
        float alpha=clampf(std::fabs(w)/3.2f,0.035f,0.24f);
        Color base=rec?ORANGE:WHITE;
    ...
    std::string stats="I "+std::to_string(brainActiveInputCount(g))+"/"+std::to_string(cfg::INPUT_COUNT)+
        ...
        " | reg "+std::to_string(brainRegisterCapacity(g))+
...
void refreshEditedBrain(Creature& c) {
    ...
    // Edicao manual altera diretamente o DNA neural.
}
...
void drawCreatureTraitEditor(Simulation& sim,UIState& ui,int sw,int sh){
    ...
    if(cat==0){
        ...
        DrawText(TextFormat("conexoes ativas %d | recorrentes %d",c->compiled.activeConnections,c->compiled.recurrentConnections),(int)x,(int)y,11,Color{165,205,190,255});y+=20.0f;
        DrawText(TextFormat("registradores persistentes %d/%d | pesos 100%% geneticos",brainRegisterCapacity(c->genome.brain),cfg::REGISTER_MEMORY_SLOTS),(int)x,(int)y,10,Color{220,190,115,255});y+=22.0f;
        DrawText("Editor: areas, ativacoes, registradores e conexoes ponderadas do DNA.",(int)x,(int)y,11,Color{165,175,195,255});y+=32.0f;
...
void drawBrainLabGraph(Creature& c, UIState& ui, RectF area) {
    ...
    for(int ci=0;ci<(int)g.connections.size();++ci){
        ...
        Vector2 a=endpoint(cn.srcKind,cn.src);
        Vector2 b=endpoint(cn.dstKind,cn.dst);
        const float displayWeight=cn.weight;
    ...
    if(hoverConnection>=0){
        ...
        hoverLines.push_back(std::string("Funcao ")+brainLinkFunctionName(cn.function)+(connectionIsRecurrent(g,cn)?" | LOOP":""));
        hoverLines.push_back("src "+Simulation::f2(src)+" -> func "+Simulation::f2(transformed)+" x peso "+Simulation::f2(w));
        hoverLines.push_back("contribuicao atual: "+Simulation::f2(contrib)+" | peso genetico");
...
void drawVisionLab(Simulation& sim,UIState& ui,int sw,int sh){
    ...
    for(int i=0;i<n;++i){
        ...
        if(rr.type==RayType::Egg)base=Color{238,218,150,255};
...
void drawBrainEmotionTab(Creature& c, UIState& ui, RectF body){
    ...
    for(int ci=0;ci<(int)c.genome.brain.connections.size()&&shown<10;++ci){const auto&cn=c.genome.brain.connections[(std::size_t)ci];if(!neuralConnectionEnabled(cn)||cn.srcKind!=BrainNodeKind::Input||cn.src!=emotionInput)continue;const float w=cn.weight;DrawText(TextFormat("%s  peso %+.3f | contrib %+.3f",brainDestinationLabel(cn.dstKind,cn.dst).c_str(),w,neuralSignal*w),(int)x+6,(int)y,10,Color{180,205,190,255});y+=15;++shown;}
...
void drawBrainLab(Simulation& sim, UIState& ui, int sw, int sh) {
    ...
    DrawText(TextFormat("Registradores %d/%d | pesos exclusivamente geneticos",regCap,cfg::REGISTER_MEMORY_SLOTS),(int)x,(int)y,9,Color{175,195,215,255});
    y+=18;
    ...
    if(!g.connections.empty()){
        ...
        DrawText("Tipo: SOMA ponderada (fixo)",(int)(x+connHalf+8),(int)y+8,10,Color{170,190,210,255});
        y+=34;
        ...
        DrawText(TextFormat("Peso genetico %.3f",cn.weight),(int)x,(int)y+6,10,Color{170,190,170,255});
        y+=31;
        if(changed) refreshEditedBrain(*c);
    }
...
void drawInspector(Simulation& sim, UIState& ui, int sw, int sh, bool& paused) {
    ...
    if(c){
        ...
        y=drawLabelValue(x,y,"Qualidade energia",Simulation::f2(c->energyQuality*100.0f)+"% | gasto "+Simulation::f2(c->energyConsumptionQualityMultiplier())+"x");
        ...
        y=drawLabelValue(x,y,"Aprendizado","Sem reforco: pesos mudam apenas por heranca, recombinacao e mutacao.");
...
enum class ConfigCategory : int {
    Energia=0, Movimento, Visao, Corpo, Cores, CicloVida, Reproducao, Plantas, OvosCarne, Fisica, Emocoes, CerebroMutacao, Sistema, Count
};

static const char* CONFIG_CATEGORY_NAMES[] = {
    "Energia", "Movimento", "Visao", "Corpo", "Cores", "Vida", "Reproducao",
    "Plantas", "Ovos/Carne", "Fisica", "Emocoes", "Cerebro/Mutacao", "Sistema"
};
...
void drawConfigPopup(Simulation& sim, UIState& ui, int sw, int sh) {
    ...
    DrawText("Configuracao completa da simulacao",(int)p.x+18,(int)p.y+14,20,RAYWHITE);
    DrawText("Valores e REGRAS alteram a simulacao. O cerebro evolui somente por DNA, recombinacao e mutacao.",(int)p.x+18,(int)p.y+40,11,Color{175,185,205,255});
    const float headerExtra=0.0f;
    ...
    float tabY=p.y+64.0f+headerExtra;
    ...
    RectF content{p.x+18,p.y+104+headerExtra,p.width-36,p.height-166-headerExtra};
    ...
    if(sel && visibleCircle(sel->pos, sel->genome.vision.range)){
        ...
        for(size_t i=0;i<sel->rays.size();i+=step){
            ...
            if(lod>0){
                ...
                else if(sel->rays[i].type==RayType::Carcass){col=sel->rays[i].targetColor;col.a=225;}
...
volatile std::sig_atomic_t saveRequested = 0;

void requestStop(int) { stopRequested = 1; }
void requestSave(int) { saveRequested = 1; }

std::filesystem::path dashboardStatusPath() {
    std::filesystem::path worldPath(saveio::defaultWorldSavePath());
    std::filesystem::path dir = worldPath.parent_path();
    if (dir.empty()) dir = std::filesystem::current_path();
    return dir / "evolucra_status.json";
}

void writeDashboardStatus(const Simulation& sim,
                          double realSeconds,
                          double actualMultiplier,
                          unsigned long long steps) {
    const auto target = dashboardStatusPath();
    ...
    out << "  \"version\": \"0.140\",\n";
    out << "  \"mode\": \"headless\",\n";
    out << "  \"learning_mode\": \"neuroevolution_only\",\n";
    out << "  \"requested_speed_multiplier\": 5.0,\n";
    out << "  \"updated_at_epoch\": " << nowEpoch << ",\n";
    out << "  \"real_seconds\": " << realSeconds << ",\n";
    out << "  \"sim_seconds\": " << (double)sim.simTime << ",\n";
    out << "  \"speed_multiplier\": " << actualMultiplier << ",\n";
    out << "  \"steps\": " << steps << ",\n";
    out << "  \"ms_per_step\": " << sim.lastUpdateMs << ",\n";
    out << "  \"population\": {\n";
    out << "    \"creatures\": " << sim.creatures.size() << ",\n";
    out << "    \"plants\": " << sim.plants.size() << ",\n";
    out << "    \"carcasses\": " << sim.carcasses.size() << ",\n";
    out << "    \"eggs\": " << sim.eggs.size() << "\n";
    out << "  },\n";
    out << "  \"limits\": {\n";
    out << "    \"min_creatures\": " << sim.limits.minCreatures << ",\n";
    out << "    \"max_creatures\": " << sim.limits.maxCreatures << ",\n";
    out << "    \"min_plants\": " << sim.limits.minPlants << ",\n";
    out << "    \"max_plants\": " << sim.limits.maxPlants << ",\n";
    out << "    \"max_enabled\": " << (sim.limits.maxEnabled ? "true" : "false") << "\n";
    out << "  },\n";
    const auto& e = sim.evo;
    out << "  \"evolution\": {\n";
    out << "    \"births\": " << e.naturalBirths << ",\n";
    out << "    \"deaths\": " << e.deaths << ",\n";
    out << "    \"natural_age_deaths\": " << e.naturalAgeDeaths << ",\n";
    out << "    \"starvation_deaths\": " << e.starvationDeaths << ",\n";
    out << "    \"attack_deaths\": " << e.attackDeaths << ",\n";
    out << "    \"eggs_laid\": " << e.eggsLaid << ",\n";
    out << "    \"eggs_laid_solo\": " << e.eggsLaidSolo << ",\n";
    out << "    \"eggs_laid_mated\": " << e.eggsLaidMated << ",\n";
    out << "    \"eggs_hatched\": " << e.eggsHatched << ",\n";
    out << "    \"eggs_destroyed\": " << e.eggsDestroyed << ",\n";
    out << "    \"minimum_replacement_eggs\": " << e.minimumReplacementEggs << ",\n";
    out << "    \"plant_births\": " << e.plantBirths << ",\n";
    out << "    \"max_generation\": " << e.maxGeneration << ",\n";
    out << "    \"avg_generation\": " << e.avgGeneration << ",\n";
    out << "    \"avg_age\": " << e.avgAge << ",\n";
    out << "    \"oldest_age\": " << e.oldestAge << ",\n";
    out << "    \"avg_body_size\": " << e.avgBodySize << ",\n";
    out << "    \"avg_connections\": " << e.avgConnections << ",\n";
    out << "    \"avg_hidden\": " << e.avgHidden << ",\n";
    out << "    \"avg_loops\": " << e.avgLoops << ",\n";
    out << "    \"avg_memory\": " << e.avgMemory << ",\n";
    out << "    \"avg_vision_focus\": " << e.avgVisionFocus << ",\n";
    out << "    \"avg_energy_ratio\": " << e.avgEnergyRatio << ",\n";
    out << "    \"avg_vision_range\": " << e.avgVisionRange << ",\n";
    out << "    \"species_count\": " << e.speciesCount << ",\n";
    out << "    \"detached_plants\": " << e.detachedPlants << "\n";
    out << "  }\n";
    out << "}\n";
    out.flush();
    out.close();
...
bool saveHeadlessSession(const Simulation& sim, const char* reason) {
    ...
    if (worldOk && configOk) {
        std::fprintf(stderr, "[HEADLESS] save %s: OK -> %s\n", reason ? reason : "", saveio::defaultWorldSavePath().c_str());
        return true;
    }
    std::fprintf(stderr, "[HEADLESS] save %s: ERRO: %s\n", reason ? reason : "", error.c_str());
    return false;
}

int run() {
    using Clock = std::chrono::steady_clock;
    constexpr double TARGET_SPEED = 5.0;
    ...
    std::fprintf(stderr,
        "[HEADLESS] Vida Artificial v0.140 - neuroevolucao pura (sem aprendizado por reforco).\n"
        "[HEADLESS] Limites: bichos=30..4000 | plantas=10..30000 | velocidade alvo=5x.\n"
        "[HEADLESS] Ctrl+C encerra com save. SIGUSR1 salva sem parar. Autosave real: %.0f s.\n",
        (double)cfg::tuning.autosaveIntervalSeconds);
    std::fflush(stderr);

    Simulation sim;
    // Em producao, reinicios do servico devem CONTINUAR do save atual.
    // Em ambiente de teste do dashboard, sem save, nasce um mundo novo.
    if (std::filesystem::exists(saveio::defaultWorldSavePath())) {
        std::string error;
        ConfigFileLoadReport cfgReport{};
        if (std::filesystem::exists(saveio::defaultConfigPath())) {
            if (!readConfigTextFile(saveio::defaultConfigPath(), cfgReport, error)) {
                std::fprintf(stderr, "[HEADLESS] aviso: config nao carregada: %s\n", error.c_str());
                error.clear();
            }
        }
        saveio::SessionMeta loadedMeta{};
        if (sim.loadFromFile(saveio::defaultWorldSavePath(), loadedMeta, error)) {
            std::fprintf(stderr, "[HEADLESS] save existente carregado: sim=%.1fs, bichos=%zu, plantas=%zu.\n",
                (double)sim.simTime, sim.creatures.size(), sim.plants.size());
        } else {
            std::fprintf(stderr, "[HEADLESS] ERRO ao carregar save existente: %s\n", error.c_str());
            return 2;
        }
    }

    sim.limits.minCreatures = 30;
    sim.limits.maxCreatures = 4000;
    sim.limits.minPlants = 10;
    sim.limits.maxPlants = 30000;
    sim.limits.maxEnabled = true;
    sim.ensureMinimums();
    sim.refreshEvolutionStats();
    sim.updateSensors();
    writeDashboardStatus(sim, 0.0, 0.0, 0);

    const float step = std::max(0.000001f, cfg::tuning.maxSubstep);
    const auto realStart = Clock::now();
    const double simStart = (double)sim.simTime;
    ...
    while (!stopRequested) {
        ...
        const double realElapsed = std::chrono::duration<double>(nowBefore - realStart).count();
        const double targetSimTime = simStart + realElapsed * TARGET_SPEED;

        // Executa apenas ate ficar em dia com 5x. Se a VM for mais lenta, roda no maximo possivel.
        int burst = 0;
        while (!stopRequested && (double)sim.simTime + 1e-9 < targetSimTime && burst < 256) {
            sim.update(step);
            ++steps;
            ++burst;
        }
        ...
        if (saveRequested) {
            saveRequested = 0;
            saveHeadlessSession(sim, "remoto");
        }

        if (std::chrono::duration<double>(now - lastStatus).count() >= 1.0) {
            const double realSeconds = std::max(0.000001, std::chrono::duration<double>(now - realStart).count());
            const double actualMultiplier = ((double)sim.simTime - simStart) / realSeconds;
            std::fprintf(stderr,
                "[HEADLESS] real=%7.1fs | sim=%9.1fs | %.2fx/5.00x | bichos=%zu plantas=%zu carne=%zu ovos=%zu | geracao=%d | nascimentos=%lld mortes=%lld | %.3f ms/step\n",
                realSeconds, (double)sim.simTime, actualMultiplier,
                sim.creatures.size(), sim.plants.size(), sim.carcasses.size(), sim.eggs.size(),
                sim.evo.maxGeneration, sim.evo.naturalBirths, sim.evo.deaths, sim.lastUpdateMs);
            std::fflush(stderr);
            writeDashboardStatus(sim, realSeconds, actualMultiplier, steps);
            lastStatus = now;
        }

        const float autosaveSeconds = cfg::tuning.autosaveIntervalSeconds;
        if (autosaveSeconds > 0.0f && std::chrono::duration<double>(now - lastAutosave).count() >= autosaveSeconds) {
            saveHeadlessSession(sim, "automatico");
            lastAutosave = now;
        }

        // Se estamos adiantados em relacao ao alvo de 5x, devolve CPU para a VM.
        const double realAfter = std::chrono::duration<double>(Clock::now() - realStart).count();
        const double desiredRealForCurrentSim = ((double)sim.simTime - simStart) / TARGET_SPEED;
        const double sleepSeconds = desiredRealForCurrentSim - realAfter;
        if (sleepSeconds > 0.0005)
            std::this_thread::sleep_for(std::chrono::duration<double>(std::min(0.01, sleepSeconds)));
        else if (burst == 0)
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--headless") == 0) headless = true;
            ...
            std::fprintf(stderr, "Uso: %s [--headless]\n  --headless  roda somente a simulacao, sem janela/renderizacao.\nNo Linux o modo headless e automatico.\n", argv[0]);
            return 0;
        }
    }
    if (headless) return headless_runtime::run();

    SetConfigFlags(FLAG_WINDOW_RESIZABLE | FLAG_VSYNC_HINT | FLAG_MSAA_4X_HINT);
    InitWindow(1280, 760, "Vida Artificial - EVOLUCAO NEURAL v0.140");
    SetTargetFPS(60);
    ...
    while(!WindowShouldClose()) {
        ...
        auto saveCheckpoint = [&](){
            ...
            try{
                ...
                std::ostringstream stem;stem<<"evolucra_mundo_t"<<(long long)std::llround(sim.simTime)<<"_g"<<sim.evo.maxGeneration;
        ...
        if(showHud && !renderModal){
            ...
            button(controls.config,"Config",ui.configPopup);
